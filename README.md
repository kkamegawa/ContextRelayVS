...
- **Streaming** — with **Stream chat responses** enabled, the reply is shown incrementally while it arrives. While a response is generating, the composer's single primary button reads **Stop** and cancels generation, so it stays reachable from the same keyboard position; stopping never issues an automatic continuation request.
