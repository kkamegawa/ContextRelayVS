...
- **Streaming** — with **Stream chat responses** enabled, the reply is shown incrementally while it arrives. While a response is generating, the send button becomes **Stop**, which cancels generation; stopping never issues an automatic continuation request.
