...
internal sealed class ClearCacheCommand : Command
{
    private readonly ContextRelayHost host;

    public ClearCacheCommand(VisualStudioExtensibility extensibility, ContextRelayHost host)
        : base(extensibility)
    {
        this.host = host;
    }
    ...
    {
        await host.ClearCacheAsync(cancellationToken).ConfigureAwait(false);
    }
}
