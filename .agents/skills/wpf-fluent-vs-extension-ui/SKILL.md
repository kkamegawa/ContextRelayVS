...
For Remote UI data contracts, the implementation MUST NOT assume that object-reference identity is preserved when synchronizing `SelectedItem`. Unless identity preservation is explicitly verified, drive the rendered selection through a serialized scalar `SelectedIndex` or a serialized per-item selected-state property, and verify that the resulting container's `IsSelected` state actually changes.
