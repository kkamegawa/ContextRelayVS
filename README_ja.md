...
- 通常の Copilot チャット — スラッシュコマンドなしの入力で Microsoft 365 Copilot の会話を開始・継続する。検索結果を暗黙的に付加せず、ピン留めスニペット、保留中の添付ファイル、`#file` メンション、有効化した場合の保存済みアクティブ エディターという明示的コンテキストのみを送る。`/ask` と異なり、明示的コンテキストがなくても実行できる
...
通常のチャットと `/ask` は同じ明示的コンテキスト規則を使います。`/ask` は利用できる明示的コンテキストがない場合、API 要求を送る前に拒否しますが、通常のチャットはいずれの場合でも実行されます。
...
- **アクティブ エディター** — **アクティブなエディターを添付** (Attach active editor) を有効にすると、アクティブ エディターの保存済み内容を添付します。選択範囲がある場合は選択した行だけを送ります。
- **グラウンディング** — 明示的コンテキストを含む要求には、添付ファイルとピン留めスニペットを主な情報源として扱う指示を付加し、その要求では Copilot の Web コンテキストを無効化します。明示的コンテキストがない要求はそのまま送信します。
