...
- Options のラベルも同じ Visual Studio の表示言語を使用し、設定変更時はプロパティ ディスクリプターを更新します。Options で保存した言語は、アウトオブプロセスのホストが共有設定ファイルを監視して開いたままのツール ウィンドウへ反映します（`auto` ではホストのロケールを含めて言語状態を再読み込みし、ウィンドウを開き直さずに ViewModel を更新します）。Visual Studio の表示言語自体は共有ユーザー設定には保存しません。
