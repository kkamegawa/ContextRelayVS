...
- Options のラベルも同じ Visual Studio の表示言語を使用し、設定変更時はプロパティ ディスクリプターを更新します。Options で保存した言語は、アウトオブプロセスのホストが共有設定ファイルを監視して開いたままのツール ウィンドウへ反映します（`auto` ではホストのロケールを含めて言語状態を再読み込みし、ウィンドウを開き直さずに ViewModel を更新します）。この監視の調停ロジック（再読み込み中に届いた保存を取りこぼさないこと、失敗した再読み込みの後も次の保存を取り漏らさないこと）は、ホストに直接書かず、単体テスト可能な小さな `ReloadCoalescer` 型に切り出しています。画面に表示中のステータス文言（件数などの結果メッセージ）は、引数を持たない固定のステータス リソースから生成されたものに限り新しい言語で再構築します。件数や例外メッセージを含む文言は、誤った表示を避けるためそのまま維持します。Visual Studio の表示言語自体は共有ユーザー設定には保存しません。
